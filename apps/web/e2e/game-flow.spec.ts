...
test.describe("Game Flow", () => {
  ...
  test("should open create room modal", async ({ page }) => {
    ...
    await expect(dialog.getByText("Connect 4")).toBeVisible();
    await expect(dialog.getByText("Rock Paper Scissors")).toBeVisible();
  });
  ...
  test("should select game and show mode options", async ({ page }) => {
    ...
    // Click Connect 4 within the modal dialog
    const dialog = page.getByRole("dialog");
    await dialog.getByText("Connect 4").click();
  ...
  test("should show Catan in create room modal", async ({ page }) => {
    ...
    const dialog = page.getByRole("dialog");
    await expect(dialog.getByText("Catan")).toBeVisible();
  });
  ...
  test("should show Splendor in create room modal", async ({ page }) => {
    ...
    const dialog = page.getByRole("dialog");
    await expect(dialog.getByText("Splendor")).toBeVisible();
  });
});
...
test.describe("Home Page Games", () => {
  test("should show all six games", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "Splendor" })).toBeVisible();
  });
