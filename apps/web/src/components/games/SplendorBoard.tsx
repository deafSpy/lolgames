...
export function SplendorBoard({
  ...
  const bankGemRefs = useRef<Record<string, HTMLDivElement | null>>({});
  const playerGemRefs = useRef<Record<string, HTMLDivElement | null>>({});
  const prevBankRef = useRef<Bank | null>(null);
  const prevPlayerGemsRef = useRef<Record<string, number> | null>(null);
  ...
  const canTakeGems = () => {
    ...
    const distinct = Object.keys(selectedPreviewGems).filter(
      (k) => (selectedPreviewGems[k as GemType] || 0) > 0
    ).length;
  ...
  const handleBankGemClick = useCallback(
    (gem: GemType) => {
      if (!isMyTurn || phase !== "take_gems") return;

      setSelectedPreviewGems((prev) => {
        const current = prev[gem] || 0;
        const otherGems = Object.keys(prev).filter(
          (k) => k !== gem && (prev[k as GemType] || 0) > 0
        );
  ...
  const handleReserveCard = useCallback(
    (cardId: string, tier: number) => {
      onAction("reserve_card", { cardId, tier, fromDeck: false });
    },
    [onAction]
  );

  const handleReserveFromDeck = useCallback(
    (tier: number) => {
      ...
      setDiscardGems((prev) => {
        ...
        const maxVal =
          gem === "gold"
            ? myPlayer?.gemGold || 0
            : (myPlayer?.[
                `gem${gem.charAt(0).toUpperCase()}${gem.slice(1)}` as keyof SplendorPlayer
              ] as number) || 0;
  ...
  const handleSelectNoble = useCallback(
    (nobleId: string) => {
  ...
  useEffect(() => {
    const allGems = ["white", "blue", "green", "red", "black", "gold"] as const;

    const currentPlayerGems: Record<string, number> = {};
    if (myPlayer) {
      for (const gem of allGems) {
        const key = `gem${gem.charAt(0).toUpperCase()}${gem.slice(1)}` as keyof SplendorPlayer;
        currentPlayerGems[gem] = myPlayer[key] as number;
      }
    }

    if (!prevBankRef.current || !prevPlayerGemsRef.current) {
      prevBankRef.current = { ...bank };
      prevPlayerGemsRef.current = { ...currentPlayerGems };
      return;
    }
    ...
    const newFlying: FlyingGem[] = [];
    let staggerIndex = 0;

    for (const gem of allGems) {
      const bankDiff = bank[gem] - prevBankRef.current[gem];
      const playerDiff = currentPlayerGems[gem] - (prevPlayerGemsRef.current[gem] || 0);
      const bankEl = bankGemRefs.current[gem];
      const playerEl = playerGemRefs.current[gem];
      if (!bankEl || !playerEl) continue;

      const bankRect = bankEl.getBoundingClientRect();
      const playerRect = playerEl.getBoundingClientRect();

      if (bankDiff < 0 && playerDiff > 0) {
        const count = Math.min(Math.abs(bankDiff), playerDiff);
        for (let i = 0; i < count; i++) {
          newFlying.push({
            id: `${gem}-to-player-${Date.now()}-${staggerIndex}`,
            gemType: gem,
            fromX: bankRect.left + bankRect.width / 2,
            fromY: bankRect.top + bankRect.height / 2,
            toX: playerRect.left + playerRect.width / 2,
            toY: playerRect.top + playerRect.height / 2,
            startDelay: staggerIndex * 80,
          });
          staggerIndex++;
        }
      } else if (bankDiff > 0 && playerDiff < 0) {
        const count = Math.min(bankDiff, Math.abs(playerDiff));
        for (let i = 0; i < count; i++) {
          newFlying.push({
            id: `${gem}-to-bank-${Date.now()}-${staggerIndex}`,
            gemType: gem,
            fromX: playerRect.left + playerRect.width / 2,
            fromY: playerRect.top + playerRect.height / 2,
            toX: bankRect.left + bankRect.width / 2,
            toY: bankRect.top + bankRect.height / 2,
            startDelay: staggerIndex * 80,
          });
          staggerIndex++;
        }
    ...
    if (newFlying.length > 0) {
      setFlyingGems((prev) => [...prev, ...newFlying]);
    }

    prevBankRef.current = { ...bank };
    prevPlayerGemsRef.current = { ...currentPlayerGems };
  }, [bank, players]); // eslint-disable-line react-hooks/exhaustive-deps
        ...
        <div className="flex gap-4 w-full">
          ...
          <div className="w-80 flex flex-col gap-3">
            ...
            {myPlayer && (
              <div className="bg-surface-800 rounded-xl p-3">
                    ...
                    {(["white", "blue", "green", "red", "black"] as const).map((gem) => {
                      ...
                      return count > 0 ? (
                        <div
                          ...
                          style={{
                            backgroundColor: GEM_COLORS[gem] + "40",
                            color: GEM_COLORS[gem],
                            border: `1px solid ${GEM_COLORS[gem]}`,
                          }}
                        >
                          {count}
                        </div>
                ...
                {myPlayer.reserved.length > 0 && (
                  <div>
                    <div className="text-xs font-semibold text-surface-400 mb-1">
                      Reserved ({myPlayer.reserved.length}/3)
                    </div>
                    <div className="flex gap-1 flex-wrap">
            ...
            {otherPlayers.map((player) => (
              <div key={player.id} className="bg-surface-800/70 rounded-xl p-3">
                <div className="flex justify-between items-center mb-2">
                  <h3 className="text-sm font-bold text-surface-300 truncate">
                    {player.displayName}
                  </h3>
          ...
          <div className="flex-1 flex flex-col gap-4">
            ...
            <div className="bg-surface-800/30 rounded-xl p-3">
              <h3 className="text-xs font-bold text-surface-400 mb-2">
                ...
                {nobles && nobles.length > 0 ? (
                  nobles.map((noble) => (
                    <motion.div
                      ...
                      onClick={() =>
                        phase === "select_noble" && isMyTurn && handleSelectNoble(noble.id)
                      }
                    ...
                    >
                      ...
                      <span className="font-black text-yellow-400 text-2xl leading-none">
                        {noble.points}
                      </span>
            ...
            <div className="flex gap-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(["white", "blue", "green", "red", "black", "gold"] as const).map((gem) => (
                    <motion.div
                      key={gem}
                      ref={(el) => {
                        bankGemRefs.current[gem] = el as HTMLDivElement | null;
                      }}
                      whileHover={
                        gem !== "gold" && isMyTurn && phase === "take_gems" ? { scale: 1.05 } : {}
                      }
                      ...
                      style={{
                        backgroundColor:
                          gem !== "gold" && isMyTurn && phase === "take_gems"
                            ? GEM_COLORS[gem] + "80"
                            : GEM_COLORS[gem] + "50",
                      }}
                    >
                      <span className="text-xl font-black drop-shadow-lg text-black">
                        {bank[gem]}
                      </span>
                    </motion.div>
                  ))}
              ...
              {isMyTurn &&
                phase === "take_gems" &&
                Object.values(selectedPreviewGems).some((v) => v) && (
            ...
            {phase === "discard_gems" && isMyTurn && (
              <div className="bg-warning/20 rounded-xl p-4 text-center border border-warning">
                <p className="text-warning mb-3 font-bold">
                  You have {getMyGemCount()} gems. Discard down to 10.
                </p>
                <div className="flex gap-2 justify-center mb-3 flex-wrap">
                  {(["white", "blue", "green", "red", "black", "gold"] as const).map((gem) => (
                    <div
                      key={gem}
                      className="flex items-center gap-2 bg-surface-800 px-3 py-2 rounded"
                    >
                      <div
            ...
            {flyingGems.map((fg) => (
              <motion.div
                ...
                style={{
                  ...
                  height: 32,
                  borderRadius: "50%",
                  backgroundColor: GEM_COLORS[fg.gemType],
                  display: "flex",
                  alignItems: "center",
                  ...
                  zIndex: 9999,
                  fontSize: "1rem",
                  boxShadow: `0 0 10px 3px ${GEM_COLORS[fg.gemType]}99`,
                }}
                initial={{ x: fg.fromX - 16, y: fg.fromY - 16, opacity: 1, scale: 1.3 }}
                animate={{ x: fg.toX - 16, y: fg.toY - 16, opacity: 0.85, scale: 1 }}
                transition={{
                  duration: 0.4,
                  delay: fg.startDelay / 1000,
                  ease: [0.25, 0.46, 0.45, 0.94],
                }}
                onAnimationComplete={() => {
                  setFlyingGems((prev) => prev.filter((g) => g.id !== fg.id));
                }}
              >
                {GEM_ICONS[fg.gemType]}
              </motion.div>
            ))}
          </AnimatePresence>,
          document.body
...
function CardDisplay({
  ...
  return (
    <motion.div
      ...
      style={{
        backgroundColor: GEM_COLORS[card.gemType] + "30",
        borderColor: GEM_COLORS[card.gemType],
        borderWidth: 2,
      }}
    ...
    >
      ...
      <div
        className={`absolute bottom-1 left-1 right-1 flex gap-0.5 flex-wrap ${tiny ? "text-[11px]" : "text-base"}`}
      >
        {card.costWhite > 0 && (
          <span
            ...
            style={{
              color: GEM_COLORS.white,
              textShadow:
                "1px 1px 2px black, -1px -1px 1px black, 1px -1px 1px black, -1px 1px 1px black",
            }}
          >
            {card.costWhite}
          </span>
        ...
        {card.costBlack > 0 && (
          <span
            ...
            style={{
              color: "#1f2937",
              textShadow:
                "1px 1px 2px rgba(255,255,255,0.8), -1px -1px 1px rgba(255,255,255,0.8), 1px -1px 1px rgba(255,255,255,0.8), -1px 1px 1px rgba(255,255,255,0.8)",
            }}
          >
            {card.costBlack}
          </span>
